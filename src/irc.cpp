...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BonPeKaOTEST2\r");
            Send(hSocket, "WHO #BonPeKaOTEST2\r");
        } else {
            // randomly join #BonPeKaO00-#BonPeKaO05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #BonPeKaO%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BonPeKaO%02d\r", channel_number).c_str());
        }
